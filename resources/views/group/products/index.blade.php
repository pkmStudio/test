...
@section('title', 'Товары категории: {{ $group->name }}')

@section('aside')
    <ul>
        @foreach($children as $category)
            @include('group.partials.groupItemSubs', ['group' => $category, 'currentGroup' => $group])
        @endforeach
    </ul>
@endsection

@section('content')
    <h1>Товары категории: {{ $group->name }}</h1>

    <!-- Форма для сортировки -->
    <form method="GET" action="{{ route('group.index.products', ['group' => $group->id]) }}" class="mb-4">
        <div class="row">
            <div class="col-md-6">
